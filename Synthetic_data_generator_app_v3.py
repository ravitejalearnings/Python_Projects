        ...
        if col["dtype"] == "number":
            with col4:
                col["min"] = st.number_input(
                    f"Min ({col['name']})", value=int(col.get("min", 0.0)), step=1, key=f"min_{table_name}_{i}")
            with col5:
                col["max"] = st.number_input(
                    f"Max  ({col['name']})", value=int(col.get("max", 0.0)), step=1, key=f"max_{table_name}_{i}")
        elif col["dtype"] == "float":
            with col4:
                col["min"] = st.number_input(
                    f"Min ({col['name']})", value=float(col.get("min", 0.0)), step=0.01, key=f"min_{table_name}_{i}")
            with col5:
                col["max"] = st.number_input(
                    f"Max  ({col['name']})", value=float(col.get("max", 0.0)), step=0.01, key=f"max_{table_name}_{i}")
        elif col["dtype"] == "date":
            with col4:
